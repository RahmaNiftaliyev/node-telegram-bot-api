...
- **Uploads stream.** Multipart bodies are hand-rolled as a web `ReadableStream` and handed straight to `fetch` (`duplex: "half"`), so file bytes flow from their source without ever being buffered - upload memory stays flat regardless of file size. `fromPath()` returns a disk-backed `Blob` (`fs.openAsBlob`, Node >= 19.8; older Nodes fall back to reading the file). `Blob`/`Uint8Array` uploads re-stream on retry; a one-shot `ReadableStream` `InputFile` is sent exactly once and a failure surfaces immediately instead of retrying; a stream factory (`InputFileStreamFactory`, `() => ReadableStream`) opens a fresh stream per attempt and stays retryable. A runtime whose `fetch` cannot stream a request body (Bun, currently) transparently sends the same bytes as one buffered `Blob`. `inputFileToBlob` is gone - nothing converts to `FormData` anymore. The default per-request `timeoutMs` is now 300000 (5 min, was 30s) so a large upload on a slow link is not cut off mid-stream.
